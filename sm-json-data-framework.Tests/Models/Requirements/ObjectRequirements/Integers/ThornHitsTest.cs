...
namespace sm_json_data_framework.Tests.Models.Requirements.ObjectRequirements.Integers
{
    public class ThornHitsTest
    {
        ...
        public void CtorFromUnfinalized_SetsPropertiesCorrectly()
        {
            ...
            // Expect
            ThornHits thornHits = model.Rooms["Kraid Room"].Links[2].To[3].Strats["Base"].Requires.LogicalElement<ThornHits>(0);
            Assert.Equal(1, thornHits.Hits);
        }
        ...
        public void CalculateDamage_Suitless_ReturnsCorrectValue()
        {
            // Given
            SuperMetroidModel model = ReusableModel();
            ThornHits thornHits = model.Rooms["Kraid Room"].Links[2].To[3].Strats["Base"].Requires
                .LogicalElement<ThornHits>(0, thornHits => thornHits.Hits == 1);
            InGameState inGameState = model.CreateInitialGameState();

            // When
            int result = thornHits.CalculateDamage(model, inGameState);
        ...
        public void Execute_NotEnoughEnergy_Fails()
        {
            ...
            // When
            ExecutionResult result = thornHits.Execute(model, inGameState);
        ...
        public void Execute_EnoughEnergy_Succeeds()
        {
            ...
            ThornHits thornHits = model.Rooms["Kraid Room"].Links[2].To[3].Strats["Base"].Requires
                .LogicalElement<ThornHits>(0, thornHits => thornHits.Hits == 1);
            InGameState inGameState = model.CreateInitialGameState()
                            .ApplyConsumeResource(ConsumableResourceEnum.Energy, 82);
        ...
        public void Execute_MultipleTimes_ConsumesCorrectAmount()
        {
            ...
            // When
            ExecutionResult result = thornHits.Execute(model, inGameState, times: 3);

            // Expect
            new ExecutionResultValidator(model, inGameState)
                .ExpectResourceVariation(RechargeableResourceEnum.RegularEnergy, -48)
                .AssertRespectedBy(result);
        }
        ...
        public void ApplyLogicalOptions_LessPossibleEnergyThanBestCaseDamage_SetsLogicalProperties()
        {
            ...
            ThornHits thornHits = model.Rooms["Kraid Room"].Links[2].To[3].Strats["Base"].Requires
                .LogicalElement<ThornHits>(0, thornHits => thornHits.Hits == 1);
            Assert.True(thornHits.LogicallyRelevant);
            Assert.True(thornHits.LogicallyNever);
            Assert.False(thornHits.LogicallyAlways);
            Assert.False(thornHits.LogicallyFree);
        }
        ...
        public void ApplyLogicalOptions_NormalPossibleEnergy_SetsLogicalProperties()
        {
            ...
            ThornHits thornHits = model.Rooms["Kraid Room"].Links[2].To[3].Strats["Base"].Requires
                .LogicalElement<ThornHits>(0, thornHits => thornHits.Hits == 1);
            Assert.True(thornHits.LogicallyRelevant);
            Assert.False(thornHits.LogicallyNever);
            Assert.False(thornHits.LogicallyAlways);
            Assert.False(thornHits.LogicallyFree);
        }

...
namespace sm_json_data_framework.Models.InGameStates
{
    ...
    public class InRoomState : ReadOnlyInRoomState
    {
        ...
        public InRoomState ApplyVisitNode(int nodeId, string stratName)
        {
            ...
            CurrentNode.LinksTo.TryGetValue(nodeId, out LinkTo linkTo);
            if (linkTo == null)
            {
        ...
        public InRoomState ApplyVisitNode(RoomNode node, Strat strat)
        {
            ...
            if (spawnOngoing && VisitedRoomPath.Any() && CurrentNode.OverrideSpawnAtNode != node)
            {
                throw new ArgumentException($"Spawn is ongoing at node {CurrentNode.Id} of room '{CurrentRoom.Name}', and that node forces the next " +
                    $"node visit to be at node {CurrentNode.OverrideSpawnAtNode.Id}");
            }

            if (strat != null)
            {
                CurrentNode.LinksTo.TryGetValue(node.Id, out LinkTo link);
                if (link == null)
                {
                    throw new ArgumentException($"There is no link from current node {CurrentNode.Id} to target node {node.Id} in room '{CurrentRoom.Name}'");
                }
    ...
    /// Represents the logically-relevant parts of the state of the current room.
    /// </summary>
    public class UnfinalizedInRoomState : ReadOnlyUnfinalizedInRoomState
    {
        public UnfinalizedInRoomState(UnfinalizedRoomNode initialNode)
        {
            ApplyEnterRoom(initialNode);
        }

        public UnfinalizedInRoomState(UnfinalizedInRoomState other)
        {
            InternalVisitedRoomPath = other.InternalVisitedRoomPath.Select(pair => (new UnfinalizedInNodeState(pair.nodeState), pair.strat)).ToList();
            InternalDestroyedObstacleIds = new HashSet<string>(other.InternalDestroyedObstacleIds);
        }

        public UnfinalizedInRoomState Clone()
        {
            return new UnfinalizedInRoomState(this);
        }

        public ReadOnlyUnfinalizedInRoomState AsReadOnly()
        {
            return this;
        }


        protected UnfinalizedInNodeState InternalCurrentNodeState { get => InternalVisitedRoomPath.Any() ? InternalVisitedRoomPath.Last().nodeState : null; }

        public ReadOnlyUnfinalizedInNodeState CurrentNodeState { get { return InternalCurrentNodeState?.AsReadOnly(); } }
        ...
        public bool BypassedExitLock
        {
            get
            {
                UnfinalizedInNodeState nodeState = InternalCurrentNodeState;
                return nodeState != null && nodeState.BypassedLocks.Any();
            }
        }
        ...
        protected List<(UnfinalizedInNodeState nodeState, UnfinalizedStrat strat)> InternalVisitedRoomPath { get; } = new List<(UnfinalizedInNodeState, UnfinalizedStrat)>();

        public IReadOnlyList<(ReadOnlyUnfinalizedInNodeState nodeState, UnfinalizedStrat strat)> VisitedRoomPath
        {
            get
            {
                return InternalVisitedRoomPath.Select<(UnfinalizedInNodeState nodeState, UnfinalizedStrat strat), (ReadOnlyUnfinalizedInNodeState nodeState, UnfinalizedStrat strat)>
                    (pair => (pair.nodeState.AsReadOnly(), pair.strat)).ToList().AsReadOnly();
            }
        }
        ...
        /// <param name="entryNode">The node by which the room is being entered.</param>
        /// <returns>This, for chaining</returns>
        public UnfinalizedInRoomState ApplyEnterRoom(UnfinalizedRoomNode entryNode)
        {
            ClearRoomState();
        ...
        /// thrown if there is no link from current node to target, or if no strat from that link is provided.</exception>
        /// <exception cref="InvalidOperationException">If this is called while this InRoomState has no current room</exception>
        public UnfinalizedInRoomState ApplyVisitNode(int nodeId, string stratName)
        {
            if(CurrentRoom == null)
            {
                throw new InvalidOperationException("Cannot try to visit a node by ID because there is no current room defined");
        ...
        /// <exception cref="ArgumentException">Thrown if a strat is provided when the node visit is part of spawning in the room; If visit is not part of spawning, 
        /// thrown if there is no link from current node to target, or if no strat from that link is provided.</exception>
        public UnfinalizedInRoomState ApplyVisitNode(UnfinalizedRoomNode node, UnfinalizedStrat strat)
        {
            // Spawning in the room is ongoing if no nodes have been visited, or only one node has been visited and that node spawns Samus elsewhere
            bool spawnOngoing = !VisitedRoomPath.Any() || (VisitedRoomPath.Count == 1 && CurrentNode.SpawnsAtDifferentNode);
            ...
            InternalVisitedRoomPath.Add((new UnfinalizedInNodeState(node), strat));
        ...
        /// <exception cref="ArgumentException">Thrown if the obstacle is not in the current room</exception>
        /// <exception cref="InvalidOperationException">Thrown if this state is not at a node (and hence has no room)</exception>
        public UnfinalizedInRoomState ApplyDestroyObstacle(string obstacleId)
        {
            if (CurrentRoom == null)
            {
                throw new InvalidOperationException("Cannot destroy an obstacle when not in a room");
        ...
        /// <exception cref="ArgumentException">Thrown if the obstacle is not in the current room</exception>
        /// <exception cref="InvalidOperationException">Thrown if this state is not at a node (and hence has no room)</exception>
        public UnfinalizedInRoomState ApplyDestroyObstacle(UnfinalizedRoomObstacle obstacle)
        {
            if (CurrentRoom == null)
            {
                throw new InvalidOperationException("Cannot destroy an obstacle when not in a room");
        ...
        /// <param name="obstacle">The obstacle to destroy.</param>
        /// <returns>This, for chaining</returns>
        protected UnfinalizedInRoomState ApplyDestroyObstacleSafe(UnfinalizedRoomObstacle obstacle)
        {
            InternalDestroyedObstacleIds.Add(obstacle.Id);
            return this;
        }
        ...
        /// <returns>This, for chaining</returns>
        /// <exception cref="InvalidOperationException">Thrown if this state is not at a node</exception>
        public UnfinalizedInRoomState ApplyOpenLock(string lockName)
        {
            if(InternalCurrentNodeState == null)
            {
                throw new InvalidOperationException("Cannot open a lock when not at a node");
        ...
        /// <returns>This, for chaining</returns>
        /// <exception cref="InvalidOperationException">Thrown if this state is not at a node</exception>
        public UnfinalizedInRoomState ApplyOpenLock(UnfinalizedNodeLock nodeLock)
        {
            if (InternalCurrentNodeState == null)
            {
                throw new InvalidOperationException("Cannot open a lock when not at a node");
        ...
        /// <returns>This, for chaining</returns>
        /// <exception cref="InvalidOperationException">Thrown if this state is not at a node</exception>
        public UnfinalizedInRoomState ApplyBypassLock(string lockName)
        {
            if (InternalCurrentNodeState == null)
            {
                throw new InvalidOperationException("Cannot bypass a lock when not at a node");
        ...
        /// <returns>This, for chaining</returns>
        /// <exception cref="InvalidOperationException">Thrown if this state is not at a node</exception>
        public UnfinalizedInRoomState ApplyBypassLock(UnfinalizedNodeLock nodeLock)
        {
            if (InternalCurrentNodeState == null)
            {
                throw new InvalidOperationException("Cannot bypass a lock when not at a node");
    ...
    /// <summary>
    /// Exposes the read-only portion of an <see cref="UnfinalizedInRoomState"/>.
    /// </summary>
    public interface ReadOnlyUnfinalizedInRoomState
    {
        /// <summary>
        /// Creates and returns a copy of this InRoomState, as a full-fledged modifiable one.
        /// </summary>
        ...
        /// The logically-relevant information about Samus' current visit to the current node.
        /// </summary>
        public ReadOnlyUnfinalizedInNodeState CurrentNodeState { get; }
        ...
        /// This strat can be null for nodes visited during the process of spawning in the room (always the first node, and sometimes the second).
        /// </summary>
        public IReadOnlyList<(ReadOnlyUnfinalizedInNodeState nodeState, UnfinalizedStrat strat)> VisitedRoomPath { get; }

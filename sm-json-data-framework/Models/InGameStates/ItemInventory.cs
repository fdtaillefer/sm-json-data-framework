...
namespace sm_json_data_framework.Models.InGameStates
{
    ...
    public class ItemInventory : ReadOnlyItemInventory
    {
        ...
        public ItemInventory(UnfinalizedItemInventory unfinalizedInventory, ModelFinalizationMappings mappings)
        {
            InternalNonConsumableItems = unfinalizedInventory.NonConsumableItems.Values.Select(item => item.Finalize(mappings)).ToDictionary(item => item.Name);
            InternalExpansionItems = unfinalizedInventory.ExpansionItems.Values
                .Select(pair => (item: pair.item.Finalize(mappings), count: pair.count))
                .ToDictionary(pair => pair.item.Name);
            InternalDisabledItemNames = new HashSet<string>(unfinalizedInventory.DisabledItemNames);

            InternalBaseResourceMaximums = unfinalizedInventory.BaseResourceMaximums.Clone();

            // Can't clone ResourceCapacityChanges because it's not accessible...
            ResourceCapacityChanges = new ResourceCount();
            foreach (var itemAndCount in InternalExpansionItems.Values)
            {
                ResourceCapacityChanges.ApplyAmountIncrease(itemAndCount.item.Resource, itemAndCount.item.ResourceAmount * itemAndCount.count);
            }
        }
    ...
    /// <para>An ItemInventory has no care for current resource counts, but is aware of maximum resource values.</para>
    /// </summary>
    public class UnfinalizedItemInventory : ReadOnlyUnfinalizedItemInventory
    {
        /// <summary>
        /// Creates and returns an inventory representing the vanilla starting inventory.
        /// </summary>
        /// <param name="model">Model from which the Item instances for starting implicit items will be obtained.</param>
        /// <returns>The inventory</returns>
        public static UnfinalizedItemInventory CreateVanillaStartingInventory(UnfinalizedSuperMetroidModel model)
        {
            return new UnfinalizedItemInventory(ResourceCount.CreateVanillaBaseResourceMaximums())
                .ApplyAddItem(model.Items[SuperMetroidModel.POWER_BEAM_NAME])
                .ApplyAddItem(model.Items[SuperMetroidModel.POWER_SUIT_NAME]);
        }
        ...
        /// </summary>
        /// <param name="baseResourceMaximums">The base maximum for all resources</param>
        public UnfinalizedItemInventory(IEnumerable<ResourceCapacity> baseResourceMaximums)
        {
            ResourceCapacityChanges = new ResourceCount();
            InternalBaseResourceMaximums = new ResourceCount(baseResourceMaximums);
        }
        ...
        /// <param name="baseResourceMaximums">A ResourceCount containing the base maximums.
        /// This instance will be cloned and will never be modified as a result of being passed here.</param>
        public UnfinalizedItemInventory(ResourceCount baseResourceMaximums)
        {
            InternalBaseResourceMaximums = baseResourceMaximums.Clone();
            ResourceCapacityChanges = new ResourceCount();
        }
        ...
        /// <param name="otherInventory">The other inventory, on which most data will be based</param>
        /// <param name="baseResourceMaximums">The base resource maximums to use instead of the one from otherInventory</param>
        public UnfinalizedItemInventory(UnfinalizedItemInventory otherInventory, ReadOnlyResourceCount baseResourceMaximums) : this(otherInventory)
        {
            InternalBaseResourceMaximums = baseResourceMaximums.Clone();
        }
        ...
        /// <param name="baseResourceMaximums">The base resource maximums to use for the clone</param>
        /// <returns>The created clone</returns>
        public UnfinalizedItemInventory WithBaseResourceMaximums(ReadOnlyResourceCount baseResourceMaximums)
        {
            return new UnfinalizedItemInventory(this, baseResourceMaximums);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public ReadOnlyUnfinalizedItemInventory AsReadOnly()
        {
            return this;
        }
        ...
        /// <param name="itemName">Name of the item to disable</param>
        /// <returns>This, for chaining</returns>
        public UnfinalizedItemInventory ApplyDisableItem(UnfinalizedItem item)
        {
            return ApplyDisableItem(item.Name);
        }
        ...
        /// <param name="itemName">Name of the item to disable</param>
        /// <returns>This, for chaining</returns>
        public UnfinalizedItemInventory ApplyDisableItem(string itemName)
        {
            if(InternalNonConsumableItems.ContainsKey(itemName))
            {
                InternalDisabledItemNames.Add(itemName);
        ...
        /// <param name="itemName">Name of the item to enable</param>
        /// <returns>This, for chaining</returns>
        public UnfinalizedItemInventory ApplyEnableItem(UnfinalizedItem item)
        {
            return ApplyEnableItem(item.Name);
        }
        ...
        /// <param name="itemName">Name of the item to enable</param>
        /// <returns>This, for chaining</returns>
        public UnfinalizedItemInventory ApplyEnableItem(string itemName)
        {
            if (InternalNonConsumableItems.ContainsKey(itemName))
            {
                InternalDisabledItemNames.Remove(itemName);
        ...
        public UnfinalizedItemInventory ExceptIn(ReadOnlyUnfinalizedItemInventory other)
        {
            // Create a new empty inventory
            UnfinalizedItemInventory returnInventory = new UnfinalizedItemInventory(InternalBaseResourceMaximums);
            // Keeping disabled items makes no sense given what this method does
            returnInventory.InternalDisabledItemNames.Clear();
    ...
    /// <summary>
    /// A read-only interface for an <see cref="UnfinalizedItemInventory"/>, allowing consultation without modification.
    /// </summary>
    public interface ReadOnlyUnfinalizedItemInventory
    {
        /// <summary>
        /// Creates and returns a copy of this ItemInventory.
        /// </summary>

...
namespace sm_json_data_framework.Models.Requirements
{
    ...
    /// A model that contains the result of executing an IExecutable.
    /// </summary>
    public class UnfinalizedExecutionResult
    {
        public UnfinalizedExecutionResult(UnfinalizedInGameState resultingState)
        {
            ResultingState = resultingState;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public UnfinalizedExecutionResult Clone()
        {
            UnfinalizedExecutionResult clone = new UnfinalizedExecutionResult(ResultingState);
            clone.ApplySubsequentResult(this);
        ...
        /// The in-game state that resulted from the execution.
        /// </summary>
        public UnfinalizedInGameState ResultingState { get; set; }
        ...
        /// <param name="subsequentResult">The result of a subsequent execution, done on this result's resultingState.</param>
        /// <returns>This ExecutionResult</returns>
        public UnfinalizedExecutionResult ApplySubsequentResult(UnfinalizedExecutionResult subsequentResult)
        {
            ResultingState = subsequentResult.ResultingState;
            RunwaysUsed = RunwaysUsed.Concat(subsequentResult.RunwaysUsed);
            CanLeaveChargedExecuted = CanLeaveChargedExecuted.Concat(subsequentResult.CanLeaveChargedExecuted);
        ...
        {
            KilledEnemies = KilledEnemies.Append(new UnfinalizedIndividualEnemyKillResult(enemy, new[] { (weapon, shots) }));
        }
        ...
        {
            KilledEnemies = KilledEnemies.Append(new UnfinalizedIndividualEnemyKillResult(enemy, killMethod));
        }
        ...
        /// 0 means current room, 3 means go back 3 rooms (using last known state), negative values are invalid. Non-playable rooms are skipped.</param>
        /// <returns>The new state of this if execution succeeds, or null in case of failure.</returns>
        public UnfinalizedExecutionResult AndThen(IExecutableUnfinalized executable, UnfinalizedSuperMetroidModel model, int times = 1, int previousRoomCount = 0)
        {
            UnfinalizedExecutionResult result = executable.Execute(model, ResultingState, times: times, previousRoomCount: previousRoomCount);
            if (result == null)
            {
                return null;
            }
    ...
    /// Expresses the killing of an enemy, along with the combination of weapons used (with the number of shots per weapon).
    /// </summary>
    public class UnfinalizedIndividualEnemyKillResult
    {
        public UnfinalizedIndividualEnemyKillResult(UnfinalizedEnemy enemy, IEnumerable<(UnfinalizedWeapon weapon, int shots)> killMethod)
        {
            Enemy = enemy;
            KillMethod = killMethod;
        }

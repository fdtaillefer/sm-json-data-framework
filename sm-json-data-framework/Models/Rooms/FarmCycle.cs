...
namespace sm_json_data_framework.Models.Rooms
{
    ...
    public class UnfinalizedFarmCycle : AbstractUnfinalizedModelElement<UnfinalizedFarmCycle, FarmCycle>, InitializablePostDeserializableInRoomEnemy
    {
        ...
        /// (whenever in-room state is relevant).</param>
        /// <returns></returns>
        public bool IsFree(UnfinalizedSuperMetroidModel model, ReadOnlyUnfinalizedInGameState inGameState, int times = 1, int previousRoomCount = 0)
        {
            // Execute the requirements of this cycle once
            UnfinalizedExecutionResult executionResult = RequirementExecution.Execute(model, inGameState, times: times, previousRoomCount: previousRoomCount);
        ...
        // Requirements execution isn't really needed. It's just an indirection to Requires.Execute().
        // It'll stay here for now just to be more consistent with the presence of Farm execution
        IExecutableUnfinalized _requirementsExecution = null;
        /// <summary>
        /// An IExecutable that corresponds to executing the requirements for this farm cycle once, without grabbing any drops.
        /// </summary>
        public IExecutableUnfinalized RequirementExecution
        {
            get
            {
                if (_requirementsExecution == null)
                {
                    _requirementsExecution = new RequirementExecutionUnfinalized(this);
                }
                return _requirementsExecution;
            }
        }

        IExecutableUnfinalized _farmExecution = null;
        /// <summary>
        /// <para>An IExecutable that corresponds to farming this group of enemies, by camping its spawner(s), killing it repeatedly, and grabbing the drops.
        /// This is repeated until all qualifying resources are filled.</para>
        /// <para>Qualifying resources are determined based on logical options.</para>
        /// <para>For simplicity, a farm execution will be considered a failure if it results in any kind of resource tradeoff.</para>
        /// </summary>
        public IExecutableUnfinalized FarmExecution
        {
            get
            {
                if (_farmExecution == null)
                {
                    _farmExecution = new FarmExecutionUnfinalized(this);
                }
                return _farmExecution;
            }
        }
    ...
    /// A class that encloses the execution of a farm cycle's requirements (for one cycle)  in an IExecutable interface.
    /// </summary>
    internal class RequirementExecutionUnfinalized : IExecutableUnfinalized
    {
        private UnfinalizedFarmCycle FarmCycle { get; set; }

        public RequirementExecutionUnfinalized(UnfinalizedFarmCycle farmCycle)
        {
            FarmCycle = farmCycle;
        }
    ...
    /// A class that encloses executing a refill of some resources using a FarmCycle in an IExecutable interface.
    /// </summary>
    internal class FarmExecutionUnfinalized : IExecutableUnfinalized
    {
        private UnfinalizedFarmCycle FarmCycle { get; set; }

        public FarmExecutionUnfinalized(UnfinalizedFarmCycle farmCycle)
        {
            FarmCycle = farmCycle;
        }
        public UnfinalizedExecutionResult Execute(UnfinalizedSuperMetroidModel model, ReadOnlyUnfinalizedInGameState inGameState, int times = 1, int previousRoomCount = 0)
        ...
        /// <param name="resourcesToRefill">The resources that should be refilled.</param>
        /// <returns></returns>
        private UnfinalizedExecutionResult ExecuteRefill(UnfinalizedSuperMetroidModel model, ReadOnlyUnfinalizedInGameState inGameState, IEnumerable<ConsumableResourceEnum> resourcesToRefill)
        {
